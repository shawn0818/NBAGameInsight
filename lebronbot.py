...
class NBAConfig:
    """Configuration class for holding API URLs, headers, and other settings."""
...
class GameDataFetcher:
    ...
    """获取playbyplay回合的视频下载连接""" 
    def get_video(self,action_number):

        url = 'https://stats.nba.com/stats/videoeventsasset'

        params = {
            'GameID': str(self.game_id),
            'GameEventID': str(action_number)
        }
...
class GameDataDisplay:  
    ...
    @staticmethod 
    def video_to_gif(viedo_url_list):
        #输出文件夹
        output_folder = NBAConfig.VIDEO_TO_PICTURE_PATH
        for idx, video_url in enumerate(viedo_url_list, start=1):
            output_path = f"{output_folder}/{idx}.gif"
            download_command = f'ffmpeg -i "{video_url}" -vf "fps=10,scale=480:-1:flags=lanczos" -c:v gif "{output_path}"'
            #调用系统里存在的 ffmpeg
...
class GameDataVisualization:
    ...
    def normalize_scores(score_df):
        """这个函数负责矩形面积的大小"""
        score_df['Score_Normalized'] = score_df['statistics_points'] / score_df['statistics_points'].sum()
        return score_df
...
class NBADataProcessor:
    ...
    def process_game_data(self):
        ...
        game_info_text = GameDataDisplay(self.game_info_df).display_game_info() 
    ...
    def process_player_data(self,player_id):
        ...
        player_text_display = GameDataDisplay(self.game_info_df).display_player_statistic(player_id,player_df)  
    ...
    def process_playbyplay_data(self,player_id):
        ...
        play_df = GameDataDisplay(self.game_info_df).display_playbyplay(player_id,actions_df)     
...
game_date = "2024-01-08"  # 假设这是需要查询的比赛日期  
player_id = "2544"
...
def main():   
    ...
    #展示比赛基本信息
    game_info_text = processor.process_game_data()
    print(game_info_text)
    player_text = processor.process_player_data(player_id)
    print(player_text)
    player_action = processor.process_playbyplay_data(player_id)
    print(player_action)
