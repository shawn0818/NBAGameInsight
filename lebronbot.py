...
class GameDataParser:
    ...
    def parse_game_data(self,boxscore_data):
        ...
        #需要添加关于球队的列的信息
        team_columns_df = game_boxsocre_df.loc[:, ['game_homeTeam_teamId', 'game_homeTeam_teamName','game_awayTeam_teamId', 'game_awayTeam_teamName']]
        #简化下列名
        team_columns_df.columns = team_columns_df.columns.str.extract(r'game_(.*?)_(.*)').agg('_'.join, axis=1)
        
        game_board_df = pd.concat([game_info_df, team_columns_df], axis=1)
             
        return game_board_df
                
    #解析关于球队的数据，比如球队整体的数据统计       
    def parse_team_data(self,boxscore_data):
    ...
    #解析球员的实时数据统计      
    def parse_player_data(self, boxscore_data):
    ...
    def parse_playbyplay_data(self, actions_data):
...
class GameDataDisplay:
    ...
    def display_game_info(game_info_parsed):
        ...
        if game_info_dict["gameStatus"] == 1:
            game_info_text = (
            "【比赛信息】 \n"
            f"时间：北京时间 {game_info_dict['gameTimeBeijing']}\n"
            f"地点：{game_info_dict['arenaCity']}，{game_info_dict['arenaName']}\n"
            f"比赛双方：{game_info_dict['homeTeam_teamName']} vs {game_info_dict['awayTeam_teamName']}"
            )        
        elif game_info_dict["gameStatus"] == 2 :          
            game_info_text = f"比赛已经在北京时间 {game_info_dict['gameTimeBeijing']}开始了，快打开电视收看吧"                
        else:
            game_info_text = f"比赛已经在北京时间 {game_info_dict['gameTimeBeijing']}开始了，现在已经结束啦"                      
    ...
    def display_realtime_score(score_data_parsed):    
        ...
        player_id = None
        player_df = score_data_parsed  
        player_filter = player_df["personId"] == int(player_id)
        one_player_df = player_df[player_filter]
    ...
    def display_gameover_score(score_data_parsed):
        ...
        team_id = None
        team_df = score_data_parsed  
        team_filter =team_df["teamId"] == int(team_id)
        one_team_df = team_df[team_filter]
        return one_team_df
...
class NBADataProcessor:
    ...
    def process_and_present_game_data(game_date, team_id):
        ...
        team_schedule_dict = TeamScheduleDataParser.get_schedule_as_dict(team_schedule_df)
        #得到 gameid
        game_id = team_schedule_dict["gameId"]
        game_boxsocre = GameDataFetcher(game_id).fetch_game_boxscore()
        game_info_df = GameDataParser().parse_game_data(game_boxsocre)
        game_info_text = GameDataDisplay().display_game_info(game_info_df)      
        return game_info_text
 
def main():
    team_id = '1610612747'  # 假设这是一个有效的球队 ID
    game_date = "2023-12-08"  # 假设这是需要查询的比赛日期
    processor = NBADataProcessor()
    #展示比赛基本信息
    game_info_text = processor.process_and_present_game_data(game_date,team_id)
