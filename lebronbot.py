...
class NBASchedule:
    ...
    def filter_team_schedule(self, team_id=None, game_date=None) -> pd.DataFrame:
        ...
        all_team_schedule_df = pd.json_normalize(all_team_schedule,
                                                 record_path=["leagueSchedule", "gameDates", "games"], errors="ignore",
                                                 sep='_')
        ...
        # 创建筛选条件
        if team_id:
            team_filter = ((all_team_schedule_df['homeTeam_teamId'] == int(team_id)) | (
                    all_team_schedule_df['awayTeam_teamId'] == int(team_id)))
            team_schedule_df = all_team_schedule_df[team_filter]
            team_schedule_df = team_schedule_df.copy()
            # 判断主场客场，生成新的列
            team_schedule_df['home_or_away'] = np.where(team_schedule_df['homeTeam_teamId'] == int(team_id), 'home',
                                                        'away')
    ...
    def fetch_data(self, endpoint):
        ...
        with self.session_manager() as session:
            try:
                ...
                print(f"Error fetching NBA metadata : {e}")
                return None

    ### 获取 playbyplay 的 json 数据
...
class GameDataParser:
    ...
    def parse_game_data(boxscore_data):
        game_boxscore_df = pd.json_normalize(boxscore_data, errors="ignore", sep='_')
        game_start_position = game_boxscore_df.columns.get_loc('game_gameId')
        game_end_position = game_boxscore_df.columns.get_loc('game_officials')
    ...
    def parse_team_data(boxscore_data):
        game_boxscore_df = pd.json_normalize(boxscore_data, errors="ignore", sep='_')
        home_start_position = game_boxscore_df.columns.get_loc('game_homeTeam_teamId')
        home_end_position = game_boxscore_df.columns.get_loc('game_homeTeam_statistics_twoPointersPercentage')
        home_team_df = game_boxscore_df.iloc[:, home_start_position:home_end_position + 1]
        away_start_position = game_boxscore_df.columns.get_loc('game_awayTeam_teamId')
        away_end_position = game_boxscore_df.columns.get_loc('game_awayTeam_statistics_twoPointersPercentage')
        away_team_df = game_boxscore_df.iloc[:, away_start_position:away_end_position + 1]
        # 修改列名一致，方便进行下一步拼接
    ...
    def parse_player_data(boxscore_data):
        # 解析出球员数据
        home_player_df = pd.json_normalize(boxscore_data, record_path=["game", "homeTeam", "players"],
                                           meta=[["game", "homeTeam", "teamId"], ["game", "homeTeam", "teamName"]],
                                           errors="ignore", sep='_')
        away_player_df = pd.json_normalize(boxscore_data, record_path=["game", "awayTeam", "players"],
                                           meta=[["game", "awayTeam", "teamId"], ["game", "awayTeam", "teamName"]],
                                           errors="ignore", sep='_')
        # 使 meta 部分添加的列名保持一致
        home_player_df = home_player_df.rename(
            columns={'game_homeTeam_teamId': 'teamId', 'game_homeTeam_teamName': 'teamName'})
        away_player_df = away_player_df.rename(
            columns={'game_awayTeam_teamId': 'teamId', 'game_awayTeam_teamName': 'teamName'})
        # 进行上下拼接
    ...
    def parse_playbyplay_data(actions_data):
        if actions_data:
            actions_df = pd.json_normalize(actions_data, record_path=['game', 'actions'], meta=[["game", "gameId"]],
                                           errors="ignore", sep='_')
            return actions_df
...
class GameDataDisplay:
    ...
    def __init__(self, game_info_parsed):
        # 首先转换时间
        game_info_parsed["gameTimeBeijing"] = pd.to_datetime(game_info_parsed["gameTimeUTC"]).dt.tz_convert(
    ...
    def display_player_statistic(self, player_id, player_data_parsed):
        ...
        player_name = the_player_dict["name"]
        # jersey_number = the_player_dict["jerseyNum"]
        status = the_player_dict["status"]
        is_played = the_player_dict["played"]  # "1"表示摸上球了
        not_playing_reason = the_player_dict["notPlayingDescription"]
        points = the_player_dict["statistics_points"]
        rebounds = the_player_dict["statistics_reboundsTotal"]
        assists = the_player_dict["statistics_assists"]
        blocks = the_player_dict["statistics_blocks"]
        steals = the_player_dict["statistics_steals"]
        plus_minus_points = int(the_player_dict["statistics_plusMinusPoints"])
        on_court_minutes = the_player_dict["statistics_minutesCalculated"][2:4]
        ...
        if status == "ACTIVE":
            ...
            player_text = f"本场比赛{player_name}不会上场，{not_playing_reason}"
    ...
    def display_playbyplay(self, player_id, actions_data_parsed):
        # 将时间转换为北京时间
        actions_data_parsed["timeActualBeijing"] = pd.to_datetime(actions_data_parsed["timeActual"]).dt.tz_convert(
            'Asia/Shanghai').dt.strftime('%Y-%m-%d %H:%M:%S')
        # 筛选特定球员的动作
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_action_number = {
                executor.submit(GameVideoDownload.get_video_url, game_id, action_number): action_number for
                action_number in action_numbers}
    ...
    def get_actions_gif(self, player_id, actions_data_parsed):
        ...
        # 视频并转换为 GIF
        GameVideoDownload.from_video_to_gif(video_info_list)
...
class NBADataProcessor:
    def __init__(self, game_date, team_id):
        ...
        self.game_boxscore = GameDataFetcher(self.game_id).fetch_game_boxscore()
        self.game_info_df = GameDataParser().parse_game_data(self.game_boxscore)
    ...
    def process_player_data(self, player_id):
        player_df = GameDataParser().parse_player_data(self.game_boxscore)
    ...
    def process_playbyplay_data(self, player_id):
        ...
        self.game_display.display_playbyplay(player_id, actions_df)

    def process_playbyplay_gif(self, player_id):
        play_action = GameDataFetcher(self.game_id).fetch_game_playbyplay()
        actions_df = GameDataParser().parse_playbyplay_data(play_action)
        self.game_display.get_actions_gif(player_id, actions_df)

    def process_treemap_data(self, team_id, player_id):
        player_df = GameDataParser().parse_player_data(self.game_boxscore)
        GameDataVisualization.display_player_scores(team_id, player_id, player_df)


def main():
    team_id = '1610612747'  # 假设这是一个有效的球队 ID
    game_date = "2024-01-18"  # 假设这是需要查询的比赛日期
    player_id = "2544"
    processor = NBADataProcessor(game_date, team_id)
    # 展示比赛基本信息
    # processor.process_game_data()
    # processor.process_player_data(player_id)
    processor.process_playbyplay_gif(player_id)
    processor.process_treemap_data(team_id, player_id)
