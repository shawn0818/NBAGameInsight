...
class TeamProfile(BaseModel):
    ...
    @classmethod
    def get_team_info(cls, identifier: Union[int, str]) -> Optional['TeamProfile']:
        """获取球队详细信息 (移动过来的方法，并进行调整)"""
        try:
            if isinstance(identifier, int):
                return cls.get_team_by_id(identifier)  # 直接调用已有的 get_team_by_id
            else:
                team_id_result = get_team_id(identifier)  # 使用 team_model.py 中的 get_team_id
                if team_id_result:
                    return cls.get_team_by_id(team_id_result[0])  # 调用 get_team_by_id 获取 TeamProfile
            return None
        except Exception as e:
            import logging  # 局部导入 logger
            logger = logging.getLogger(__name__)  # 获取当前模块的 logger
            logger.error(f"获取球队信息时出错: {e}", exc_info=True)
            return None